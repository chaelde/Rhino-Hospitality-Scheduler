...
export default function ManagerDashboard() {
  ...
  const addEmployee = async () => {
    ...
    try {
      ...
      alert(
        `Employee created! Instructions have been sent to the employee via email.`
      );
      setNewEmployee({
        name: "",
        email: "",
  ...
  return (
    <div className="p-6 space-y-8 text-gray-200 bg-gray-900 min-h-screen">
        ...
        {sectionsOpen.addEmployee && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select
              ...
              onChange={(e) =>
                setNewEmployee({ ...newEmployee, location_id: e.target.value })
              }
            >
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-md">
        <div
          className="flex justify-between items-center cursor-pointer"
          onClick={() => toggleSection("timeOffRequests")}
        >
          <h2 className="text-xl font-semibold border-b border-gray-700 pb-2 mb-2">
            Time Off Requests
          </h2>
                    ...
                    {requests.map((req) => (
                      <tr key={req.id} className="hover:bg-gray-700 transition">
                        ...
                        <td className="border p-2">
                          {format(new Date(req.start_date), "MM/dd/yyyy")}
                        </td>
                        <td className="border p-2">
                          {format(new Date(req.end_date), "MM/dd/yyyy")}
                        </td>
                        <td className="border p-2">{req.reason || "-"}</td>
                        <td className="border p-2">
                          {locations.find((loc) => loc.id === req.location_id)
                            ?.name || "-"}
                        </td>
                        <td className="border p-2">
                          {req.date_submitted
                            ? format(
                                new Date(req.date_submitted),
                                "MM/dd/yyyy"
                              )
                            : "-"}
                        </td>
